...
sum ihd_combpvm		//								date of coronary heart disease diagnosis
...
local dg anyinf bactinf viralinf anycns noncns
foreach idg of local dg {
		tab any`idg'
		replace any`idg' = . if anyanyinf==1 & any`idg'==0 
		tab any`idg'
...
quietly: local irow = 7
	...
	forvalues j=1/6 {
		...
		sum diabetes_combpvm
		gen byte dm=0
		replace dm = 1 if diabetes_combpvm <= entrypvm
		tab dm
		sum hypert_combpvm
		replace hypert_combpvm = . if hypert_combpvm>=exitpvm
		sum hypert_combpvm
		gen byte htn = 0
		replace htn = 1 if hypert_combpvm <= entrypvm
		tab htn
