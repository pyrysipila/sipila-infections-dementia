...
capture log close
log using "Jakauma-datan luominen.log", append
	...
	foreach age of local age_groups {
		foreach sex of local sexes {
			...
			foreach lag of local entry_lags {
				...
				replace entry_lag = 0 if `N_`cohort'_`age'_`sex''==0 & entry_lag==. & anyanyinf==0 & cohort==`cohort' & supu==`sex' 
				}
				}
				}
	...
	foreach age of local age_groups {
		foreach sex of local sexes {
			...
			foreach lag of local entry_lags {
				...
				replace entry_lag_10y = 0 if `N_`cohort'_`age'_`sex''==0 & entry_lag_10y==. & anyanyinf==0 & cohort==`cohort' & supu==`sex'
				}
				}
				}
...
save "jakauma.dta", replace

use "jakauma.dta", clear
...
egen latedementia = cut(_t), at(0,65,100), if _d==1
tab latedementia
...
stset exitpvm, id(id) failure(ensidementia) origin(time syntpvm) enter(time lag_entry_anyinf) scale(365.25) 
	...
	count if ensidementia==1
		...
		exportexcel(eTable_3, cell(A6) replace)
...
preserve
drop if _st!=1
bootstrap incidence = r(rate), reps(1000) strata(cohort): stptime, per(100000)
estat bootstrap, all
*Percentile-based confidence intervals were reported in the manuscript.
restore
...
*Store the data to Stata format 
...
ren anycnsdate ensianycnspvm
ren noncnsdate ensianynoncnspvm

ren anycns anyanycns 
ren noncns anyanynoncns
...
ren anyanynoncns anynoncns
ren ensianynoncnspvm ensinoncnspvm
	...
	foreach age of local age_groups {
		foreach sex of local sexes {
			...
			foreach lag of local entry_lags {
				...
				replace entry_lag = 0 if `N_`age'_`sex''==0 & entry_lag==. & anyanyinf==0 & supu==`sex'
				}
				}
...
quietly: local irow = 7
	...
	forvalues j=1/6 {
		...
		sum diabetes_combpvm
		gen byte dm=0
		replace dm = 1 if diabetes_combpvm <= lag_entry_`idg'
		tab dm
		sum hypert_combpvm
		replace hypert_combpvm = . if hypert_combpvm>=exitpvm
		sum hypert_combpvm
		gen byte htn = 0
		replace htn = 1 if hypert_combpvm <= lag_entry_`idg'
		tab htn
...
replace toinenanyinf=0 if toinenanyinf==.
replace kolmasanyinf=0 if kolmasanyinf==.
...
*Difference between CNS infections and extra-CNS infections
	...
	*CNS infection vs non cns infection 
	capture drop anycnsnotcns
	capture drop lag_entry_cnsnotcns
	capture gen byte anycnsnotcns = 1 if ensianycnspvm<=ensinoncnspvm & anyanycns==1 & ensianycnspvm<exitpvm
	replace anycnsnotcns = 0 if ensinoncnspvm<ensianycnspvm & anycnsnotcns!=1 & anynoncns==1 & ensinoncnspvm<exitpvm
	capture egen ensicnsnotcnspvm = rowmin(ensianycnspvm ensinoncnspvm)
	replace ensicnsnotcnspvm = . if ensicnsnotcnspvm >= exitpvm
	replace ensicnsnotcnspvm = . if anycnsnotcns!=1 & anycnsnotcns!=0
	gen lag_entry_cnsnotcns = lag_entry_anyinf if anyanyinf==0
	replace lag_entry_cnsnotcns = ensicnsnotcnspvm if ensicnsnotcnspvm!=. & (anycnsnotcns==1 | anycnsnotcns==0) 
	count
	sum ensicnsnotcnspvm
	tab anycnsnotcns
	tab anyanycns
...
quietly: local irow = 7
	...
	forvalues j=1/6 {
		...
		scalar `ll' = m[5,1]
		scalar `ul' = m[6,1]
		quietly: putexcel set "SipilaUKB_time-dep_$S_DATE.xlsm", sheet("FIGURE UKB") modify
			quietly: putexcel K`irow'=(`e(N_sub)'), nformat(number)
			quietly: putexcel L`irow'=(`e(N_fail)'), nformat(number)
			quietly: putexcel E`irow'=(`hr'), nformat(number_d2)
			quietly: putexcel F`irow'=(`ll'), nformat(number_d2)
...
foreach outcome in AD VD FTD PD {
	...
	forvalues j=1/6 {
		...
		scalar `ll' = m[5,1]
		scalar `ul' = m[6,1]
		quietly: putexcel set "SipilaUKB_time-dep_$S_DATE.xlsm", sheet("FIGURE UKB, `outcome'") modify
			quietly: putexcel K`irow'=(`e(N_sub)'), nformat(number)
			quietly: putexcel L`irow'=(`e(N_fail)'), nformat(number)
			quietly: putexcel E`irow'=(`hr'), nformat(number_d2)
			quietly: putexcel F`irow'=(`ll'), nformat(number_d2)
...
local prow = `irow'
		...
		forvalues i =1/3 {
		quietly: putexcel set "UKB infection burden time-dep $S_DATE.xlsm", sheet("FIGURE sensitivity") modify
			quietly: putexcel K`irow'=(`e(N_sub)'), nformat(number)
			quietly: putexcel L`irow'=(`e(N_fail)'), nformat(number)
			quietly: putexcel E`irow'=(`hr`i''), nformat(number_d2)
			quietly: putexcel F`irow'=(`ll`i''), nformat(number_d2)
...
tab anyinfcount, mis
tab anyinfcount ensidementia, mis
capture gen ensiinfcountpvm=ensianyinfpvm if anyinfcount!=0 & anyinfcount!=.
...
local prow = `irow'
		...
		forvalues i =1/3 {
		quietly: putexcel set "UKB infection burden time-dep $S_DATE.xlsm", sheet("FIGURE sensitivity AD") modify
			quietly: putexcel K`irow'=(`e(N_sub)'), nformat(number)
			quietly: putexcel L`irow'=(`e(N_fail)'), nformat(number)
			quietly: putexcel E`irow'=(`hr`i''), nformat(number_d2)
			quietly: putexcel F`irow'=(`ll`i''), nformat(number_d2)
...
local prow = `irow'
		...
		forvalues i =1/3 {
		quietly: putexcel set "UKB infection burden time-dep $S_DATE.xlsm", sheet("FIGURE sensitivity VaD") modify
			quietly: putexcel K`irow'=(`e(N_sub)'), nformat(number)
			quietly: putexcel L`irow'=(`e(N_fail)'), nformat(number)
			quietly: putexcel E`irow'=(`hr`i''), nformat(number_d2)
			quietly: putexcel F`irow'=(`ll`i''), nformat(number_d2)
